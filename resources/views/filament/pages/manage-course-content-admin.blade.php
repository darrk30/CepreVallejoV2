    ...
    <div class="aula">
        ...
        <div class="aula-banner">
            ...
            <div class="aula-banner-info">
                ...
                <p>
                    {{ $this->assignment->cicloCourse->academicCycle->nombre ?? 'Ciclo activo' }}
                    · Profesor: {{ $this->assignment->teacher?->user?->name ?? 'Sin datos' }}
                    · Turno {{ $this->assignment->turno?->nombre ?? '—' }}
                </p>
            </div>
        </div>
                    ...
                    <div class="sec-card" data-id="{{ $section->id }}">
                        ...
                        <div class="sec-head">
                            ...
                            <div class="aula-actions-mobile">
                                <button class="btn-more" onclick="toggleDropdown('sec-drop-{{ $section->id }}', event)"
                                ...
                                <div class="dropdown-menu" id="sec-drop-{{ $section->id }}">
                                    <button class="dropdown-item item-sort" id="sortDetailsMobile-{{ $section->id }}"
                        ...
                        <div class="details-list" id="detailsList-{{ $section->id }}">
                            @forelse($section->details as $di => $detail)
                                <div class="detail-row" data-id="{{ $detail->id }}" data-section="{{ $section->id }}"
                                    x-data="{ openVideo: false }">

                                    <div class="drag-handle sm detail-handle-{{ $section->id }}" title="Arrastrar tema">
                                        <span></span><span></span><span></span>
                                    </div>
                                        ...
                                        <div class="detail-resources">
                                            ...
                                            @if ($detail->archivo_path)
                                                <a href="{{ Storage::url($detail->archivo_path) }}"
                                                    ...
                                                    class="res-file" title="Descargar {{ basename($detail->archivo_path) }}">
                                        ...
                                        @if ($detail->url_video)
                                            <template x-if="openVideo">
                                                <div class="video-wrap">
                                                    <iframe x-bind:src="buildEmbedUrl('{{ addslashes($detail->url_video) }}')"
                                                        allowfullscreen
                                    ...
                                    <div class="aula-actions-mobile" style="padding-top:1px;">
                                        <button class="btn-more" onclick="toggleDropdown('det-drop-{{ $detail->id }}', event)"
                                            aria-label="Más acciones">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                                                <circle cx="5" cy="12" r="1.5" />
                                                <circle cx="12" cy="12" r="1.5" />
