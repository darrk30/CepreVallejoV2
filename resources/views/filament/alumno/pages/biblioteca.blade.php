{{-- Migrado a Tailwind. Si algo falla, descomenta este bloque (y quita las
     clases Tailwind del <div class="bv">) para volver al CSS clásico. --}}
{{--
@push('styles')
    ...
    <div class="font-handlee pb-[60px] text-[#0c0b1a] dark:text-[#edecf8]">
        ...
        <div class="mb-[5px] flex flex-wrap items-center justify-between gap-5 border-b border-[#6366f1]/14 pb-2.5 dark:border-[#818cf8]/12">
            <div class="min-w-0 flex-1">
                ...
                <h1 class="m-0 text-[clamp(1.8rem,4vw,2.8rem)] font-extrabold leading-[1.1] tracking-[-0.02em] text-[#0c0b1a] dark:text-[#edecf8]">Biblioteca <em class="italic text-[#5b5ef4] dark:text-[#818cf8]">Virtual</em></h1>
                <p class="mt-2 text-[0.72rem] font-semibold text-[#8b88b0] dark:text-[#636086]">
                    {{ $this->favoritos->count() + $this->librosTotal }} título{{ ($this->favoritos->count() + $this->librosTotal) !== 1 ? 's' : '' }} disponible{{ ($this->favoritos->count() + $this->librosTotal) !== 1 ? 's' : '' }}
                </p>
            </div>
            <div class="flex h-[60px] w-[60px] shrink-0 items-center justify-center rounded-2xl border border-[#5b5ef4]/25 bg-[#5b5ef4]/7 dark:border-[#818cf8]/25 dark:bg-[#818cf8]/8">
                <svg class="h-7 w-7 text-[#5b5ef4] dark:text-[#818cf8]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if ($this->favoritos->count() > 0 && !$search && !$areaId)
            <div class="mb-[18px] flex items-center gap-3.5">
                ...
                <span class="whitespace-nowrap text-[0.67rem] font-bold uppercase tracking-[0.1em] text-[#8b88b0] dark:text-[#636086]">{{ $this->favoritos->count() }} libro{{ $this->favoritos->count() !== 1 ? 's' : '' }}</span>
            </div>
        ...
        {{-- BIBLIOTECA GENERAL ── --}}
        <div class="mb-[18px] flex items-center gap-3.5">
            <h2 class="m-0 whitespace-nowrap text-[1.2rem] font-bold text-[#0c0b1a] dark:text-[#edecf8]">📚 Biblioteca General</h2>
            <span class="h-px flex-1 bg-[#6366f1]/14 dark:bg-[#818cf8]/12"></span>
            <span class="whitespace-nowrap text-[0.67rem] font-bold uppercase tracking-[0.1em] text-[#8b88b0] dark:text-[#636086]">{{ $this->librosTotal }} títulos</span>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3.5 min-[560px]:grid-cols-3 min-[560px]:gap-[18px] min-[900px]:grid-cols-4 min-[900px]:gap-[22px] min-[1200px]:grid-cols-5">
            ...
            @empty
                <div class="col-span-full rounded-[14px] border-[1.5px] border-dashed border-[#6366f1]/28 bg-[#f9f8ff] px-8 py-[72px] text-center dark:border-[#818cf8]/28 dark:bg-[#1b1a2e]">
                    ...
                    <p class="m-0 text-[0.75rem] text-[#8b88b0] dark:text-[#636086]">No encontramos libros que coincidan con tu búsqueda.</p>
                </div>
            @endforelse
        </div>

        @if ($this->libros->count() < $this->librosTotal)
            <div class="mt-9 flex justify-center">
                <button type="button" wire:click="loadMoreLibros" wire:loading.attr="disabled" wire:target="loadMoreLibros"
                    class="rounded-full border border-[#5b5ef4]/25 bg-[#5b5ef4]/7 px-6 py-2.5 text-[0.78rem] font-bold text-[#5b5ef4] transition-colors duration-150 hover:bg-[#5b5ef4]/14 disabled:opacity-60 dark:border-[#818cf8]/25 dark:bg-[#818cf8]/8 dark:text-[#818cf8] dark:hover:bg-[#818cf8]/16">
                    <span wire:loading.remove wire:target="loadMoreLibros">Cargar más</span>
                    <span wire:loading wire:target="loadMoreLibros">Cargando…</span>
                </button>
            </div>
        @endif
